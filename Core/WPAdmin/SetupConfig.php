...
class SetupConfig
{
	...
	static function __constructStatic()
	{
		...
		switch ($step) {
			case -1:
				if (\Core\WPadmin\Includes\TranslationInstall::wp_can_install_language_pack() && empty($language)) {
					$languages = \Core\WPadmin\Includes\TranslationInstall::wp_get_available_translations();
					if ($languages) {
						self::setup_config_display_header('language-chooser');
						echo '<h1 class="screen-reader-text">Select a default language</h1>';
						echo '<form id="setup" method="post" action="?step=0">';
						\Core\WPadmin\Includes\TranslationInstall::wp_install_language_form($languages);
						echo '</form>';
						break;
					}
				}
			case 0:
				...
				self::setup_config_display_header();
				$step_1 = 'setup-config?step=1';
...
?>
			...
			?>
				...
				<form method="post" action="setup-config?step=2">
			...
			case 2:
				...
				// Re-construct $GLOBALS['wpdb'] with these new values.
				unset($GLOBALS['wpdb']);
				\Core\WPIncludes\Load::require_wp_db();
		...
		*/
				...
				if (!empty($GLOBALS['wpdb']->error)) {
					wp_die($GLOBALS['wpdb']->error->get_error_message() . $tryagain_link);
				}

				$errors = $GLOBALS['wpdb']->hide_errors();
				$GLOBALS['wpdb']->query("SELECT $prefix");
				$GLOBALS['wpdb']->show_errors($errors);
				...
				foreach ($config_file as $line_num => $line) {
					...
					switch ($constant) {
						...
						case 'DB_HOST':
							...
							break;
						case 'DB_CHARSET':
							if ('utf8mb4' === $GLOBALS['wpdb']->charset || (!$GLOBALS['wpdb']->charset && $GLOBALS['wpdb']->has_cap('utf8mb4'))) {
	...
	static function setup_config_display_header($body_classes = array())
	{
	...
	?>
		...
		<?php
	} // End function self::setup_config_display_header();
}
