...
class SetupConfig {
	...
	function __construction(){
		$this->wp_installing = true;
		$this->wp_setup_config = true;
		$this->abspath = dirname( __DIR__ ) . '/';
		$this->abspath . 'wp-settings.php';
		$this->abspath . 'wp-admin/includes/upgrade.php';
		$this->abspath . 'wp-admin/includes/translation-install.php';
		nocache_headers();
		if ( file_exists( $this->abspath . 'wp-config-sample.php' ) ) {
			$config_file = file( $this->abspath . 'wp-config-sample.php' );
		} elseif ( file_exists( dirname( $this->abspath ) . '/wp-config-sample.php' ) ) {
			$config_file = file( dirname( $this->abspath ) . '/wp-config-sample.php' );
		} else {
			wp_die(
				sprintf(
		...
		if ( file_exists( $this->abspath . 'wp-config.php' ) ) {
			wp_die(
				'<p>' . sprintf(
		...
		if ( @file_exists( $this->abspath . '../wp-config.php' ) && ! @file_exists( $this->abspath . '../wp-settings.php' ) ) {
			wp_die(
				'<p>' . sprintf(
				...
				switch ( $step ) {
					case -1:
						...
						case 0:
							...
							echo '<h1 class="screen-reader-text">' . _e( 'Before getting started' ) . '</h1>
						...
						<p>
							...
							<p>'. _e( 'In all likelihood, these items were supplied to you by your Web Host. If you don&#8217;t have this information, then you will need to contact them before you can continue. If you&#8217;re all ready&hellip;' ).'</p>
							<p class="step"><a href="'. $step_1.'" class="button button-large">'.
							_e( 'Let&#8217;s go!' ).'</a></p>
							'
							break;
...
(function(){
	if ( ! /iPad|iPod|iPhone/.test( navigator.userAgent ) ) {
		...
		*/
		if ( file_exists( $this->abspath . 'wp-config-sample.php' ) ) {
			$path_to_wp_config = $this->abspath . 'wp-config.php';
		} else {
			$path_to_wp_config = dirname( $this->abspath ) . '/wp-config.php';
		}
