...
class Option
{
	 ...
	 */
		...
		if (!Load::wp_installing()) {
			...
			if (isset($alloptions[$option])) {
				...
				$value = Cache::wp_cache_get($option, 'options');
	...
	function wp_load_alloptions($force_cache = false)
	{
		...
		if (!Load::wp_installing() || !is_multisite()) {
			$alloptions = Cache::wp_cache_get('alloptions', 'options', $force_cache);
		} else {
			$alloptions = false;
		}
	...
	function wp_load_core_site_options($network_id = null)
	{
		...
		if (!is_multisite() || wp_using_ext_object_cache() || Load::wp_installing()) {
			return;
		}
	 ...
	 */
		...
		$notoptions = Cache::wp_cache_get('notoptions', 'options'); // Yes, again... we need it to be fresh.
	...
	function get_transient($transient)
	{
		...
		if (wp_using_ext_object_cache()) {
			$value = Cache::wp_cache_get($transient, 'transient');
		} else {
	...
	function get_network_option($network_id, $option, $default = false)
	{
		...
		$notoptions     = Cache::wp_cache_get($notoptions_key, 'site-options');
		...
		if (!is_multisite()) {
			...
			$value     = Cache::wp_cache_get($cache_key, 'site-options');
	...
	function add_network_option($network_id, $option, $value)
	{
		...
		if (!is_multisite()) {
			...
			// This option exists now.
			$notoptions = Cache::wp_cache_get($notoptions_key, 'site-options'); // Yes, again... we need it to be fresh.
	...
	function get_site_transient($transient)
	{
		...
		if (wp_using_ext_object_cache()) {
			$value = Cache::wp_cache_get($transient, 'site-transient');
		} else {
