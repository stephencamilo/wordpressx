...
function wp_dashboard_setup() {
	...
	// Site Health.
	if ( current_user_can( 'view_site_health_checks' ) && ! Load::is_network_admin() ) {
	...
	// Right Now.
	if ( Load::is_blog_admin() && current_user_can( 'edit_posts' ) ) {
	...
	// Activity Widget.
	if ( Load::is_blog_admin() ) {
		wp_add_dashboard_widget( 'dashboard_activity', __( 'Activity' ), 'wp_dashboard_site_activity' );
	}

	// QuickPress Widget.
	if ( Load::is_blog_admin() && current_user_can( get_post_type_object( 'post' )->cap->create_posts ) ) {
...
function wp_dashboard_right_now() {
    ...
    <div class="main">
		...
		if ( ! Load::is_network_admin() && ! Load::is_user_admin() && current_user_can( 'manage_options' ) && '0' == get_option( 'blog_public' ) ) {
...
function wp_dashboard_quick_press( $error_msg = false ) {
	...
	if ( $last_post_id ) {
		...
		if ( in_array( Load::get_current_blog_id(), array_keys( get_blogs_of_user( $user_id ) ), true ) ) {
...
function wp_dashboard_quota() {
	if ( ! Load::is_multisite() || ! current_user_can( 'upload_files' ) || get_site_option( 'upload_space_check_disabled' ) ) {
		return true;
	}
...
function wp_dashboard_browser_nag() {
	...
	if ( $response ) {
		...
		if ( ! empty( $response['img_src'] ) ) {
			$img_src = ( Load::is_ssl() && ! empty( $response['img_src_ssl'] ) ) ? $response['img_src_ssl'] : $response['img_src'];
...
function wp_check_browser_version() {
	...
	if ( false === $response ) {
		...
		if ( Load::is_wp_error( $response ) || 200 != wp_remote_retrieve_response_code( $response ) ) {
			return false;
		}
