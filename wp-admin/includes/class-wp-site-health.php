...
class WP_Site_Health
{
	...
	{
		if (!wp_next_scheduled('wp_site_health_scheduled_check') && !Load::wp_installing()) {
			...
			foreach ($tests['direct'] as $test) {
				if (is_string($test['test'])) {
					...
					if (method_exists($this, $test_function) && is_callable(array($this, $test_function))) {
						$health_check_js_variables['site_status']['direct'][] = $this->perform_test(array(
							$this,
							$test_function
						));
						continue;
	...
	{
		return in_array(Load::wp_get_environment_type(), array('development', 'local'), true);
	}
	...
	{
		if (!Load::is_admin() || !is_user_logged_in() || !current_user_can('update_core') || !isset($_GET['health-check-test-wp_version_check'])) {
			return;
		}
	...
	public function get_test_plugin_version()
	{
		...
		if ($plugins_total > $plugins_active && !Load::is_multisite()) {
	...
	public function get_test_theme_version()
	{
		...
		if ($has_unused_themes && $show_unused_themes && !Load::is_multisite()) {
	...
	public function get_test_scheduled_events()
	{
		...
		if (Load::is_wp_error($this->has_missed_cron())) {
			$result['status'] = 'critical';
	...
	{
		if (Load::is_wp_error($this->crons)) {
			return $this->crons;
		}
	...
	public function get_test_file_uploads()
	{
		...
		if (Load::wp_convert_hr_to_bytes($post_max_size) < Load::wp_convert_hr_to_bytes($upload_max_filesize)) {
			$result['label']       = sprintf(
	...
	public function wp_cron_scheduled_check()
	{
		...
		foreach ($tests['async'] as $test) {
			...
			if (is_string($test['test'])) {
				...
				if (!Load::is_wp_error($result_fetch) && 200 === wp_remote_retrieve_response_code($result_fetch)) {
